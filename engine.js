...
(function (root, factory) {
  ...
  function classifyPage(P, cal) {
    ...
    for (const L of P.lines) {
      ...
      if (isCueLine(L, [cal.cueX - 12, cal.cueX + 12])) {
    ...
    for (const B of P.blocks) {
      ...
      for (const L of [B.cue, ...B.lines]) {
        const scaled = s > 1.001 && (pageAnchor != null
          ? L.ax0 != null
          : (L.cls === 'dialogue' && L.enlarge !== false));
  ...
  return function createSidesEngine({ pdfjsLib, PDFLib }) {
    ...
    async function process(bytes, opts = {}) {
      ...
      if (mode === 'page') {
        ...
        // unmoved baseline, spreading horizontally from the page's content
        // center, so stage direction runs edge to edge and dialogue widens by
        // the same factor. Margin furniture (left scene numbers, revision
        // stars, page numbers past pageW-80) stays put exactly. Growth is
        // capped by the page edges, by margin marks on the same line, and by
        // line spacing so ascenders and descenders of adjacent lines don't
        // collide. Baselines never move, so parity stays structural.
        const E = 10, ML = 70;
        for (let i = 0; i < pdfPages.length; i++) {
          ...
          // scene numbers sit closer than a segment gap to the slug text and
          // must never ride along with the body.
          const body = [];
          let bx0 = Infinity, bx1 = -Infinity;
          for (const L of P.lines) {
            const bs = L.items.filter(t => t.x + t.w > ML && t.x < marginR);
            if (!bs.length) { L.ax0 = L.ax1 = null; continue; }
          ...
          const anchor = (bx0 + bx1) / 2;
          pageReport.anchor = Math.round(anchor * 100) / 100;

          // width limit: page edges and same-line margin marks
          let sW = requested;
          for (const L of body) {
            if (L.ax0 < anchor) sW = Math.min(sW, (anchor - L.leftStop) / (anchor - L.ax0));
            if (L.ax1 > anchor) sW = Math.min(sW, (L.rightStop - anchor) / (L.ax1 - anchor));
          }
          // spacing limit: descender above + ascender below <= baseline gap
          // (Courier metrics ~0.157/0.629 em, held slightly safe)
          let sV = requested;
          const rows = [...body].sort((p, q) => q.y - p.y);
          for (let k = 0; k + 1 < rows.length; k++) {
            const gap = rows[k].y - rows[k + 1].y;
            if (gap < 6) continue; // same visual row (dual columns etc.)
            const su = median(rows[k].items.map(t => t.size)) || 12;
            const sl = median(rows[k + 1].items.map(t => t.size)) || 12;
            sV = Math.min(sV, gap / (0.16 * su + 0.64 * sl));
          }

          // every body line becomes an eligible "dialogue" proxy spanning its
          // body extent, anchored at the page content center
          const proxies = body.map(L => ({ y: L.y, x0: L.ax0, x1: L.ax1, dx0: L.ax0, dx1: L.ax1, cls: 'dialogue' }));
          ...
          // rect (table cells, row bands) or it gets cut off invisibly
          let sClip = requested;
          if (requested > 1.001) {
            const constraints = [];
            const mstats = { scaledOps: 0, warnings: [] };
            rewriteLevel(latin, [1, 0, 0, 1, 0, 0], pageRes, proxies, requested, anchor, mstats, new Set(), constraints);
            for (const c of constraints) {
              if (c.clipUnknown) { sClip = 1; break; }
              if (c.clipHi < Infinity && c.L.dx1 - anchor > 1) sClip = Math.min(sClip, (c.clipHi - anchor) / (c.L.dx1 - anchor));
              if (c.clipLo > -Infinity && anchor - c.L.dx0 > 1) sClip = Math.min(sClip, (anchor - c.clipLo) / (anchor - c.L.dx0));
            }
          }

          const s = Math.max(1, Math.floor(Math.min(requested, sW, sV, sClip) * 100) / 100);
          pageReport.appliedScale = s;
          if (s < requested - 0.005) {
            const binding = Math.min(sW, sV, sClip);
            pageReport.warnings.push(binding === sClip && sClip < Math.min(sW, sV)
