    ...
    for (const P of pages) {
      ...
      for (const L of P.lines) {
        ...
        if (/^(INT|EXT|I\/E|INT\/EXT|EST)[.\s\/]/.test(text)) {
          ...
          // scene number prints in BOTH margins; show it once, labeled and
          // set clear of the heading, so "- DAY     Sc. 24" doesn't read as
          // the phrase "DAY 24".
          const sc = num(live.filter(t => t.x + t.w <= ML)) || num(live.filter(t => t.x >= marginR));
          let slugText = text;
          // the scene number often bleeds into the extracted slug text (its
          // right-margin copy sits inside the body zone, or it glues onto the
          // last word); peel a trailing copy so it isn't doubled next to the
          // "Sc." label. The [^0-9] boundary keeps it from eating digits of a
          // larger number; no lookbehind (old iOS Safari would throw).
          if (sc) {
            const esc = sc.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
            slugText = slugText.replace(new RegExp('([^0-9])' + esc + '\\s*$'), '$1').trim();
          }
          push({ t: 'slug', text: slugText + (sc ? '     Sc. ' + sc : '') });
          continue;
        }
